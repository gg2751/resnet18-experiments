...
# Basic block for ResNet with Batch Normalization
...
# ResNet architecture with Batch Normalization
class ResNet(nn.Module):
    ...
    def forward(self, x):
        # Forward pass through initial layers and ResNet blocks
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
...
# Experiments without Batch Norm
# Basic block for ResNet without Batch Normalization
...
# Function to create a ResNet-18 model without Batch Normalization
def ResNet18NoBN():
